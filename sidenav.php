<div class="sidebar">
  <nav class="nav flex-column">
    <a href="#" class="nav-link navbar-brand">
      <span class="icon"><i class="bi bi-grid"></i></span>
      <span class=" description">Registrar</span>
    </a>

    <!-- module 1 -->
    <a href="#" class="nav-link text-center active" data-bs-toggle="collapse" data-bs-target="#module1drop"
      aria-expanded="false" aria-controls="module1drop">
      <i class="bi bi-file-person-fill"></i>
      <span class="description">Personal Info Database
        <i class="bi bi-caret-down-fill"></i>
      </span>
    </a>

    <!-- submodules 1 -->
    <div class="sub-menu collapse" id="module1drop">
      <a href="dashboard.php" class="nav-link"><i class="bi bi-list-ul"></i> <span
          class="description">Dashboard</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 2</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 3</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 4</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 5</span></a>
    </div>

    <!-- module 2 -->
    <a href="#" class="nav-link text-center" data-bs-toggle="collapse" data-bs-target="#module2drop"
      aria-expanded="false" aria-controls="module2drop">
      <i class="bi bi-person-lines-fill"></i> <span class="description">Guardian & Emergency Contact
        <i class="bi bi-caret-down-fill"></i>
      </span>
    </a>

    <!-- submodules 2 -->
    <div class="sub-menu collapse" id="module2drop">
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 1</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 2</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 3</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 4</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 5</span></a>
    </div>

    <!-- module 3 -->
    <a href="#" class="nav-link text-center" data-bs-toggle="collapse" data-bs-target="#module3drop"
      aria-expanded="false" aria-controls="module3drop">
      <i class="bi bi-book-half"></i>
      <span class="description">Academic History
        <i class="bi bi-caret-down-fill"></i>
      </span>
    </a>

    <!-- submodules 3 -->
    <div class="sub-menu collapse" id="module3drop">
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 1</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 2</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 3</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 4</span></a>
      <a href="#" class="nav-link"><i class="bi bi-list-ul"></i> <span class="description">sub module 5</span></a>
    </div>

    <!-- Repeat similar structure for modules 4 to 10 -->
    <!-- module 4 -->
    <a href="#" class="nav-link text-center">
      <i class="bi bi-clipboard-heart"></i>
      <span class="description">Health Record Log</span>
    </a>

    <!-- module 5 -->
    <a href="#" class="nav-link text-center">
      <i class="bi bi-qr-code"></i> <span class="description">QR Code Integration</span>
    </a>

    <!-- module 6 -->
    <a href="#" class="nav-link text-center">
      <i class="bi bi-person-badge-fill"></i>
      <span class="description">Student ID Generation</span>
    </a>

    <!-- module 7 -->
    <a href="#" class="nav-link text-center">
      <i class="bi bi-folder"></i>
      <span class="description">Document Requests</span>
    </a>

    <!-- module 8 -->
    <a href="#" class="nav-link text-center">
      <i class="bi bi-person-check"></i>
      <span class="description">Student Status Tracker</span>
    </a>

    <!-- module 9 -->
    <a href="#" class="nav-link text-center">
      <i class="bi bi-archive"></i>
      <span class="description">Digital File Storage</span>
    </a>

    <!-- module 10 -->
    <a href="#" class="nav-link text-center">
      <i class="bi bi-list-columns-reverse"></i>
      <span class="description">Student MasterList Generator</span>
    </a>

  </nav>
</div>